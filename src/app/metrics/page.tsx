...
export default function Metrics() {
  ...
  const allWebsiteTasks: WebsiteTasks = {
    ...
    'NetworkIn': [
      ...
      { id: 12, prompt: 'Message connection John Smith about Backend Developer opportunity at his startup with specific salary range', difficulty: 'Medium', type: 'retrievalAndAction', success: false },
              ...
              <tbody>
                {allWebsiteTasks[selectedWebsite].map((task) => (
                  <tr key={task.id} className="border-b border-gray-100">
                    ...
                    <td className="py-4 px-4">
                      <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium
                        ${task.type === 'retrieval' ? 'bg-pink-100 text-pink-800' :
                          task.type === 'action' ? 'bg-blue-100 text-blue-800' :
